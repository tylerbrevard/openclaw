...
it("finishes the same real ledger from the candidate after the old updater is fenced by migration", async () => {
  ...
  const display = { onStepComplete: vi.fn() };
  const progress = createUpdateRunProgress(run, display);
  ...
  expect(display.onStepComplete).toHaveBeenCalledOnce();
