...
export function createConfigWriteCoordinator({
  ...
  const runAutoSave = () => {
    ...
    void trackWrite(
      (onSubmitted) =>
        run(() =>
          submitConfigDraft(state, "auto", onSubmitted, () => canCallConfigMethod("config.set")),
        ),
      true,
    ).catch(() => undefined);
  };
    ...
    return afterPendingWritesSettled(
      async () => {
        ...
        try {
          ...
          return await patchConfig(state, resolved.options, (ack, snapshotAtDispatch) => {
      ...
      return !canDispatch()
        ...
        : afterPendingWritesSettled(
            async (onSubmitted) => {
              ...
              cancelAppliedRefresh();
              try {
                const saved = await submitConfigDraft(state, "save", onSubmitted, canDispatch);
                reconcileAutoSaveDraftConnection();
                return saved;
              } finally {
                reconcileAppliedRefresh();
